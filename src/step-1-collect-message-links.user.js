...
(function () {
  ...
  function isLikelySupportWidgetContent({
    ...
    if (html.includes("messenger-button") || html.includes("type a message"))
      return true;
    if (
      text.includes("you're back online") &&
      text.includes("outreach support")
    )
      return true;
  ...
  function isThreadMessagesRequestMeta(reqMeta) {
    const op = String(
      reqMeta?.operation_name || reqMeta?.req_operation_name || "",
    );
  ...
  function extractThreadMessagesCandidates(url, json, reqMeta = {}) {
    ...
    for (const item of col) {
      ...
      const prospectId = String(
        item?.prospectId || item?.message?.prospectId || "",
      );
      ...
      const bodyMasked =
        !!item?.bodyMasked || isBodyHiddenMarker(bodyText, bodyHtml);
      if (!messageId && !bodyText && !bodyHtml) continue;

      if (reqMessageId && !messageIdsMatchLoose(messageId, reqMessageId))
        continue;
  ...
  function extractPageSnapshot() {
    try {
      ...
      for (const el of clone.querySelectorAll(
        '[id*="messenger"],[class*="messenger"],[id*="widget"],[class*="widget"],[id*="support"],[class*="support"]',
      )) {
        el.remove();
      }
  ...
  function chooseBestCandidateForRow(row, rawCandidates) {
    ...
    function enrich(candidate, defaultSource) {
      ...
      c.hidden =
        !!c.body_masked || isBodyHiddenMarker(c.body_text, c.body_html);
      ...
      if (
        targetMessageId &&
        messageIdsMatchLoose(c.req_message_id, targetMessageId)
      )
        c.score += 480;
      if (
        targetMessageId &&
        messageIdsMatchLoose(c.message_id, targetMessageId)
      )
        c.score += 140;
      if (
        targetProspectId &&
  ...
  function getTargetedNetworkCandidateForRow(row) {
    ...
    const exact = alive.filter((c) => {
      ...
      if (c.capture_source !== "payload:Messages_GetThreadMessages")
        return false;
      if (
        !isThreadMessagesRequestMeta({ operation_name: c.req_operation_name })
      )
        return false;
    ...
    const scored = finalPool.map((c) => {
      ...
      if (
        targetMessageId &&
        messageIdsMatchLoose(candidate.req_message_id, targetMessageId)
      )
        candidate.score += 600;
      if (
        targetMessageId &&
        messageIdsMatchLoose(candidate.message_id, targetMessageId)
      )
        candidate.score += 220;
      if (
        targetProspectId &&
  ...
  async function captureRowBodyViaUi(row, timeoutMs = UI_CAPTURE_TIMEOUT_MS) {
    ...
    while (Date.now() - started < timeoutMs) {
      ...
      if (best) {
        if (deterministicSourceOk && bestMessageIdMatch && best.hidden) {
          return best;
        }
        if (
          !best.hidden &&
          ...
          bodyChars(best.body_text, best.body_html) >=
            UI_CAPTURE_MIN_GOOD_CHARS &&
          bestMessageIdMatch &&
      ...
      if (nowMs - state.lastCapturePulseAt > 4000) {
        log("UI capture pulse", {
          ...
          best_req_msgid_prefix: String(best?.req_message_id || "").slice(
            0,
            24,
          ),
        });
